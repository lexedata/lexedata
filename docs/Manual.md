...
In order to maintain the integrity of the CLDF format, which is a relational database, lexedata scripts often have to operate on multiple files. It is therefore common that a lexedata command does not have as an argument a single file. Instead, it operates on as many files of the CLDF format as necessary for the operation at hand. For more information on the CLDF format and its files, see [here](https://github.com/Anaphory/lexedata/blob/natalia-review/docs/cldf.md).
