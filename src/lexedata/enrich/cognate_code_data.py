...
def clean_segments(
    ...
    for s in range(len(segments) - 1, 0, -1):
        if not pyclts.models.is_valid_sound(segments[s - 1], bipa):
            if isinstance(segments[s - 1], pyclts.models.Marker) and \
               isinstance(segments[s], pyclts.models.Marker):
                del segments[s - 1]
...
if __name__ == '__main__':
    ...
    parser.add_argument("wordlist", default="cldf-metadata.json", type=Path,
