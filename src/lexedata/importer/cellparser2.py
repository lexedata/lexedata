...
class AbstractCellParser():
    ...
    def parse(self, values, coordinate):
        ...
        for element in values:
            try:
                parsed_dict = self.parse_value(element, coordinate)
                # replace None
...
class CellParserLexical(CellParser):
    ...
    def __init__(
            ...
            illegal_symbols_transcription: Optional[Pattern] = None,
            comment_escapes: Optional[Pattern] = None,
            add_default_source=True,
            scann_for_variants: Optional[str] = ""
    ):
...
class TupiFormCellParser(CellParserLexical):
    def __init__(
            self,
...
class CellParserCognate(TupiFormCellParser):
    def parse_value(self, values, coordinate):
        if values.isupper():
