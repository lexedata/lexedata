...
def get_headers_from_excel(
    ...
    row_property = list(row_property)
...
def read_single_excel_sheet(
    ...
    new_data = import_language_from_sheet(
        ...
        row_property_columns=concept_property_columns,
    ...
    for form, concept in new_data:
        ...
        if concept_candidates:
            for concept_id in concept_candidates:
                ...
                logger.warning(f"Concept {concept[c_c_name]} from sheet "
                               f"{language[c_l_name]} is already in the data set")
                break
        else:
        ...
        if form_candidates:
            for form_id in form_candidates:
                form[c_f_id] = form_id
                logger.warning(f"Form {form[c_f_value]} from sheet "
                               f"{language[c_l_name]} is already in the data set")
